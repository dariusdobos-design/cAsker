...
export const PIN_HEIGHT = 53;
export const BUBBLE_WIDTH = 270;
export const TAIL_HEIGHT = 14;
export const CARD_BODY_HEIGHT = 286;
export const BUBBLE_HEIGHT = CARD_BODY_HEIGHT + TAIL_HEIGHT;

/** Spätná kompatibilita s mapou */
export const CARD_WIDTH = BUBBLE_WIDTH;
export const CARD_HEIGHT = BUBBLE_HEIGHT;

const MIN_SCALE_X = PIN_WIDTH / BUBBLE_WIDTH;
const MIN_SCALE_Y = PIN_HEIGHT / BUBBLE_HEIGHT;
const OPEN_SPRING = { damping: 20, stiffness: 260, mass: 0.75 };
const CLOSE_SPRING = { damping: 24, stiffness: 320, mass: 0.7 };

type ServicePinExpandCardProps = {
...
export function ServicePinExpandCard({
  service,
  onClose,
  closeToken = 0,
}: ServicePinExpandCardProps) {
  ...
  /** Celá bublina (karta + chvost) rastie zo špičky chvosta = miesto pinu */
  const bubbleStyle = useAnimatedStyle(() => {
    const scaleX = interpolate(progress.value, [0, 1], [MIN_SCALE_X, 1]);
    const scaleY = interpolate(progress.value, [0, 1], [MIN_SCALE_Y, 1]);

    return {
      transform: [
        { translateX: BUBBLE_WIDTH / 2 },
        { translateY: BUBBLE_HEIGHT },
        { scaleX },
        { scaleY },
        { translateX: -BUBBLE_WIDTH / 2 },
        { translateY: -BUBBLE_HEIGHT },
  ...
  const pinStyle = useAnimatedStyle(() => ({
    opacity: interpolate(progress.value, [0, 0.3, 0.55], [1, 0.5, 0]),
  }));

  const bodyStyle = useAnimatedStyle(() => ({
    opacity: interpolate(progress.value, [0, 0.32, 0.6], [0, 0.2, 1]),
    borderRadius: interpolate(progress.value, [0, 1], [16, 18]),
  }));

  /** Chvost ostáva na špičke pinu počas celej animácie */
  const tailStyle = useAnimatedStyle(() => ({
    opacity: interpolate(progress.value, [0, 0.12, 1], [0.85, 1, 1]),
  }));
  ...
  return (
    <>
      <Animated.View style={[styles.bubble, bubbleStyle]}>
        ...
        <Animated.Image
          source={require("@/assets/icons/service-map-pin.png")}
          resizeMode="contain"
